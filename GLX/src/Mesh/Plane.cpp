...
Plane::Plane(VertexBuffer& vbo, float* vertices, size_t size, RendererLayer* renderer)
...
AABB Plane::generateAABB(const float* vertices, size_t size)
{
...
SphereBV Plane::generateSphereBV(const float* vertices, size_t size)
{
    glm::vec3 minAABB = glm::vec3(std::numeric_limits<float>::max());
    glm::vec3 maxAABB = glm::vec3(std::numeric_limits<float>::min());
...
void Plane::drawSelfAndChildSimple(GLenum& mode, Shader& shader)
{
    if (!children.empty()) {
...
void Plane::drawSelfAndChildTranslucent(GLenum& mode, const Frustum& frustum, unsigned int& display, unsigned int& total)
{
    if (!children.empty()) {
        total++;
        for (auto&& child : children) {
...
void Plane::drawSelfAndChild(GLenum& mode, const Frustum& frustum, unsigned int& display, unsigned int& total)
{
    if (!children.empty()) {
        total++;
        for (auto&& child : children) {
...
void Plane::renderGUI(int i)
{
    if (ImGui::TreeNodeEx(("Plane " + std::to_string(i)).c_str())) {
        ...
        if (ImGui::Button("Remove Plane", ImVec2(0, 0))) {
